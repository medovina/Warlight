...
import warlight.tournament.utils.CSV;
import warlight.tournament.utils.CSV.CSVRow;
import warlight.utils.LazyMap;

/**
 * Summarize table.csv that WarlightFight may incrementally output.
 * 
 * @author Jimmy
 */
public class WarlightTable {
    ...
    public WarlightTable(File tableFileIn, File summaryFileOut) {
    ...
    public void run() {
        if (tableFileIn.isFile()) {
            ...
            TableSummary summary = WarlightTable.readTable(tableFileIn);
            System.out.println("Writing: " + summaryFileOut.getAbsolutePath());
            WarlightTable.writeSummary(summary, summaryFileOut);
        } else 
        if (tableFileIn.isDirectory()) {
            ...
            for (File file : tableFileIn.listFiles()) {
                if (file.getName().endsWith(".csv")) {
                    ...
                    WarlightTable.readTable(file, summary);
                }
            }
